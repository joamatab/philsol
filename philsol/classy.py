...
class phil_class:
    ...
    def build_stuff(
        self, x_bound=None, y_bound=None, kx_bloch=0, ky_bloch=0, matrices=None
    ):

        if matrices is None:
            self.P, _ = ps.core.eigen_build(
                self.k0,
                self.n,
                self.dx,
                self.dy,
            ...
            self.P, self.mats = ps.core.eigen_build(
                self.k0,
                self.n,
                self.dx,
                self.dy,
    ...
    def solve_stuff(self, neigs, beta_trial, extra_fields=None):
        ...
        self.Eigs = neigs
        self.beta, self.Ex, self.Ey = ps.solve.solve(
            self.P, beta_trial, E_trial=self.E_trial, neigs=self.Eigs
        )
        ...
        if extra_fields:
            ...
            for i in range(self.Eigs):
                ...
                ez, hx, hy, hz = ps.construct.extra_feilds(
                    self.k0, self.beta[i], self.Ex[i, :], self.Ey[i, :], self.mats
                )
                self.E[i, :, 2] = ez
                self.H[i, :, 0] = hx
                self.H[i, :, 1] = hy
