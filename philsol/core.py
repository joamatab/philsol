"""
Contains the math from Zhu paper
assembles Maxwells equations into a linear eigenvalue problem:
- The synatax is as close to verbatim as we could make it
- Returns scipy.sparse matrix which defines the eigen problem for a
given refractive index profile.
- Other operator can be return to help construct other coupled fields
(such as E_z, H_x, H_y, H_z)
"""
...
def eigen_build(k0, n, dx, dy, x_boundary=None, y_boundary=None):
    ...
    t = time.time()
    Pxx = -Ux * epszi * Vy * Vx * Uy / k0 ** 2 + (k0 ** 2 * I + Ux * epszi * Vx) * (
        epsx + Vy * Uy / k0 ** 2
    )

    Pyy = -Uy * epszi * Vx * Vy * Ux / k0 ** 2 + (k0 ** 2 * I + Uy * epszi * Vy) * (
        epsy + Vx * Ux / k0 ** 2
    )

    Pxy = (
        Ux * epszi * Vy * (epsy + Vx * Ux / k0 ** 2)
        - (k0 ** 2 * I + Ux * epszi * Vx) * Vy * Ux / k0 ** 2
    )

    Pyx = (
        Uy * epszi * Vx * (epsx + Vy * Uy / k0 ** 2)
